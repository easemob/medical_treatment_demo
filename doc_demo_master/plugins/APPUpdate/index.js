...
// 弹窗图标url
const $iconUrl = "/static/icons/ic_ar.png";
const API_TOKEN = "ad90d5c3fd0d206b411b8c93e3a21979";
const APP_ID = "60ee9724b2eb46592f59eb12";
// 获取当前应用的版本号
...
const getServerNo = function(version, isPrompt = false, callback) {
	...
	/****************以下是示例*******************/
	// 可以用自己项目的请求方法
	$http.get('http://api.jappstore.com/apps/latest/' + APP_ID + "?api_token=" + API_TOKEN).then(
		res => {
			// console.log(res);
			if(res.version){
				let netVersion=parseInt(res.version);
				let curVersion=parseInt(version.versionCode);
				if (netVersion > curVersion) {
...
const getDownload = function(data) {
	...
	dtask = plus.downloader.createDownload(data.installUrl, {
...
function updatePopup(data, callback) {
	...
	// 描述的列表
	let text=data.changelog?data.changelog:"新版本需要更新";
	const descriptionList = drawtext(text, viewContentWidth);
	// 弹窗容器高度
	...
	getCurrentNo(versionInfo => {
		getServerNo(versionInfo, isPrompt, res => {
			...
			updatePopup(res, function() {
				if (/\.wgt$/i.test(res.installUrl)) {
					getDownload(res);
				} else if (/\.html$/i.test(res.installUrl)) {
					plus.runtime.openURL(res.installUrl);
				} else {
					if (platform == "android") {
						getDownload(res);
					}
